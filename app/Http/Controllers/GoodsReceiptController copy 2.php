...
use App\Models\Warehouse; // <--- TAMBAHKAN INI
...
class GoodsReceiptController extends Controller
{
    ...
    public function store(Request $request, $po_id)
    {
        $request->validate([
            ...
            // 🔥 PASTIKAN GUDANG PENERIMAAN DIKIRIM DARI FORM GR (Opsional jika form tidak ada) 🔥
        ...
        try {
            DB::transaction(function () use ($request, $po_id) {
                ...
                // 3. Proses Item GR & Sihir Stok/Aset Otomatis
                foreach ($request->items as $itemId => $data) {
                    ...
                    if ($qtyReceived > 0) {
                        ...
                        if ($qtyReceived > $sisaYangBolehDiterima) {
                            throw new \Exception("Kuantitas terima melebihi sisa pesanan PO!");
                        }
                        ...
                        if ($masterItem->is_stockable) {
                            ...
                            // =========================================================
                            // 🔥 SIHIR BATCH STOK (MULTI-GUDANG) 🔥
                            // =========================================================
                            \App\Models\InventoryStock::create([
                                'company_id'       => $po->bill_to_company_id,
                                'warehouse_id'     => $request->warehouse_id ?? 1, // Pastikan input warehouse_id ada di view GR
                                'item_id'          => $masterItem->id,
                                'stock_qty'        => $qtyReceived,
                                'reference_number' => $newGrNumber, // Nomor GR jadi identitas Batch
                                'notes'            => "PO: {$po->po_number} | Ket: {$catatanItem}", // Catatan untuk Batch
                            ]);
                            // =========================================================

                            // Simpan Mutasi (Kartu Stok Umum)
                            ...
                            // Update Total Stok di Master Item
                        ...
                        if ($masterItem->is_asset) {
                            ...
                            for ($i = 0; $i < $qtyInt; $i++) {
                                ...
                                \App\Models\FixedAsset::create([
                                    ...
                                    // 🔥 INI DIA PENYELAMATNYA: TAMBAHKAN WAREHOUSE ID 🔥
                                    'warehouse_id'     => $request->warehouse_id ?? 1,
                ...
                // 4. Update Status PO & PR Otomatis
                $po->refresh();
            ...
            return redirect()->route('gr.index')->with('success', 'Penerimaan Barang berhasil disimpan dan nomor GR telah diterbitkan! Stok/Aset otomatis terupdate.');
