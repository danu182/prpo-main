...
class GoodsReceiptController extends Controller
{
    ...
    // ========================================================
    // CREATE GR (BERDASARKAN NOMOR PO / SLUG)
    // ========================================================
    public function create($slug)
    {
        // 🔥 TAMBAHKAN 'items.item.uom' AGAR SATUAN DASAR (ECERAN) IKUT TERPANGGIL 🔥
        $po = PurchaseOrder::with(['vendor', 'items.item.itemUoms', 'items.item.uom'])->where('po_number', $slug)->firstOrFail();

        $pendingItems = $po->items->filter(function ($item) {
            $received = $item->qty_received ?? 0;
            return round($item->qty_ordered - $received, 4) > 0;
        });
   ...
   // ========================================================
    // STORE GR (BERDASARKAN NOMOR PO / SLUG)
    // ========================================================
    public function store(Request $request, $slug, \App\Services\SystemSettingService $settingService)
    {
        ...
        try {
            $newGrNumber = DB::transaction(function () use ($request, $slug, $settingService) {
                ...
                // 3. PROSES ITEM GR & SIHIR KONVERSI UOM TINGKAT DEWA
                foreach ($request->items as $itemId => $data) {
                    ...
                    if ($inputQty > 0) {
                        ...
                        // A. Tentukan faktor konversi UOM bawaan PO
                        $poConvFactor = 1;
                        $cleanPoUom = trim(preg_replace('/ \(Isi:.*\)/i', '', $poItem->uom));
                        ...
                        // B. Tentukan faktor konversi UOM yang dipilih User saat Terima Barang
                        $rawInputUom = $data['uom'] ?? $poItem->uom;
                        $cleanInputUom = trim(preg_replace('/ \(Isi:.*\)/i', '', $rawInputUom));
                        $inputConvFactor = 1;

                        // 🔥 TRIK REGEX: Langsung sedot angka dari teks (Contoh: "Pack (Isi: 40)") 🔥
                        if (preg_match('/\(Isi:\s*([0-9.]+)\)/i', $rawInputUom, $matches)) {
                            $inputConvFactor = (float) $matches[1];
                        } else {
                            if (strtolower($cleanInputUom) === strtolower($cleanPoUom)) {
                        ...
                        // C. Ubah semua angka ke Satuan Dasar (Eceran)
                        ...
                        if (round($baseQtyReceived, 4) > $sisaBaseYangBolehDiterima) {
                            throw new \Exception("Kuantitas terima ({$baseQtyReceived} Eceran) melebihi sisa pesanan PO ({$sisaBaseYangBolehDiterima} Eceran) untuk barang: {$masterItem->name}!");
                        }

                        $catatanKonversi = "Diterima: {$inputQty} {$cleanInputUom} (= {$baseQtyReceived} Eceran). " . ($data['notes'] ?? '');

                        // Simpan histori penerimaan (GR Item)
                        \App\Models\GoodsReceiptItem::create([
                            ...
                            'notes'                  => $catatanKonversi,
                        ]);

                        // D. Update catatan penerimaan di PO
                        ...
                        if ($masterItem->is_stockable) {
                            ...
                            \App\Models\InventoryStock::create([
                                ...
                                'notes'            => "PO: {$po->po_number} | {$catatanKonversi}", 
                            ]);
                        ...
                        if ($masterItem->is_asset) {
                            ...
                            for ($i = 0; $i < $qtyInt; $i++) {
                                ...
                                \App\Models\FixedAsset::create([
                                    ...
                                    'notes'            => 'Aset masuk dari GR: ' . $grNumber
                                ]);
                                $nextSeq++;
                            }
                        }
                    ...
                    if ($pr) {
                        ...
                        foreach($pr->items as $prItem) {
                            if ($prItem->status === 'APPROVED') {
                                if (round($prItem->ordered_qty ?? 0, 4) < round($prItem->qty, 4)) {
                                    $semuaDipesan = false;
                                    break;
                                }
                            }
                        ...
                        if ($semuaDipesan && $semuaPoSelesai) {
                            ...
                            if ($statusSelesaiPr && optional($pr->status)->slug !== 'completed') {
                                ...
                                \App\Models\PurchaseRequestHistory::create([
                                    ...
                                    'user_id'             => auth()->id(),
                                    'action'              => 'COMPLETED',
                                    'note'                => "Siklus selesai. Semua barang diterima penuh di Gudang (Ref PO: {$po->po_number})."
                                ]);
                            }
                        }
                    }
            ...
            return redirect()->route('gr.index')->with([
                ...
                'print_url' => route('gr.print', $newGrNumber), 
                'new_gr'    => $newGrNumber
            ]);
    ...
    // ========================================================
    // SHOW GR (BERDASARKAN NOMOR GR / SLUG)
    // ========================================================
    ...
    // ========================================================
    // PRINT GR (BERDASARKAN NOMOR GR / SLUG)
    // ========================================================
    ...
    // ========================================================
    // PRINT LABELS (BERDASARKAN NOMOR GR / SLUG)
    // ========================================================
