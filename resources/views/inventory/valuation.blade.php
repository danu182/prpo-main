...
<style>
    ...
    /* Kolom Angka Mutasi */
    .col-angka { font-size: 1rem; font-weight: 700; text-align: right; }
</style>
@endpush
...
<div class="pb-5 container-fluid px-md-4">
    ...
    <div class="gap-3 mb-4 d-flex flex-column flex-md-row justify-content-between align-items-md-end">
        <div>
            <h3 class="mb-0 fw-bold text-dark d-flex align-items-center">
                <i class="bi bi-cash-stack text-primary me-2"></i> Laporan Valuasi Inventory
            </h3>
            <p class="mt-1 mb-0 text-muted small">Lacak pergerakan stok (Qty) beserta nilai uang (Rupiah) pada periode tertentu.</p>
        </div>

        {{-- 🔥 TOMBOL EXCEL & CETAK LAPORAN (SUDAH DIARAHKAN KE VALUATION) 🔥 --}}
        <div class="gap-2 d-flex">
            <a href="{{ route('inventory.valuation.export', request()->all()) }}" class="shadow-sm btn btn-success fw-bold rounded-pill">
                <i class="bi bi-file-earmark-excel me-1"></i> Export Excel
            </a>
            <a href="{{ route('inventory.valuation.print', request()->all()) }}" target="_blank" class="shadow-sm btn btn-dark fw-bold rounded-pill">
    ...
    <div class="mb-4 bg-white border-0 border-4 shadow-sm border-top card rounded-4 border-primary">
        <div class="p-4 card-body">
            {{-- 🔥 ACTION FORM DIKEMBALIKAN KE HALAMAN INDEX VALUASI 🔥 --}}
            <form action="{{ route('inventory.valuation') }}" method="GET">
    ...
    {{-- TABEL DATA MODERN (MUTASI QTY + VALUASI RP) --}}
    <div class="mb-4 border-4 card-table-wrapper border-top border-dark">
        <div class="px-4 py-3 bg-white card-header border-bottom-0 d-flex flex-column flex-md-row justify-content-between align-items-md-center">
            <h6 class="mb-0 fw-bold text-dark">
                <i class="bi bi-box-seam me-2 text-dark"></i> Valuasi Stok:
            ...
            <table class="table align-middle table-modern text-nowrap">
                <thead>
                    <tr>
                        ...
                        <th class="text-end bg-warning-subtle text-dark border-start border-warning" width="10%">Harga Pengali<br><span class="fw-normal text-muted" style="font-size: 0.65rem; text-transform:none;">(Dasar Valuasi)</span></th>
                        <th class="text-end bg-light" width="15%">Saldo Awal<br><span class="fw-normal text-muted" style="font-size: 0.65rem; text-transform:none;">(Qty & Nilai Uang)</span></th>
                        <th class="text-end bg-success-subtle text-success" width="15%">+ Masuk<br><span class="fw-normal text-success" style="font-size: 0.65rem; text-transform:none;">(Qty & Nilai Uang)</span></th>
                        <th class="text-end bg-danger-subtle text-danger" width="15%">- Keluar<br><span class="fw-normal text-danger" style="font-size: 0.65rem; text-transform:none;">(Qty & Nilai Uang)</span></th>
                        <th class="text-end pe-4 bg-primary-subtle text-primary" width="15%">Saldo Akhir<br><span class="fw-normal text-primary" style="font-size: 0.65rem; text-transform:none;">(Qty & Nilai Uang)</span></th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($items as $index => $item)
                        <tr>
                            ...
                            <td>
                                <div class="mb-1 fw-bold text-dark text-wrap" style="max-width: 200px;">{{ $item->name }}</div>
                                <span class="border badge bg-secondary-subtle text-secondary border-secondary-subtle font-monospace">{{ $item->code }}</span>
                            </td>

                            {{-- 🔥 KOLOM KHUSUS HARGA PENGALI 🔥 --}}
                            <td class="text-end bg-warning-subtle border-start border-warning">
                                <span class="fw-bold text-dark fs-6">Rp {{ number_format($item->harga_satuan ?? 0, 0, ',', '.') }}</span>
                            </td>

                            {{-- Saldo Awal (Qty & Rp) --}}
                            <td class="text-end bg-light">
                                <div class="col-angka text-muted d-block w-100">{{ number_format($item->saldo_awal, 0, ',', '.') }}</div>
                                <div class="mt-1 text-secondary fw-medium" style="font-size: 0.75rem;">Rp {{ number_format($item->nilai_awal ?? 0, 0, ',', '.') }}</div>
                            </td>

                            {{-- Mutasi Masuk (Qty & Rp) --}}
                            <td class="text-end bg-success-subtle">
                                <div class="col-angka text-success d-block w-100">{{ $item->mutasi_in > 0 ? '+'.number_format($item->mutasi_in, 0, ',', '.') : '-' }}</div>
                                <div class="mt-1 opacity-75 text-success fw-medium" style="font-size: 0.75rem;">Rp {{ number_format($item->nilai_in ?? 0, 0, ',', '.') }}</div>
                            </td>

                            {{-- Mutasi Keluar (Qty & Rp) --}}
                            <td class="text-end bg-danger-subtle">
                                <div class="col-angka text-danger d-block w-100">{{ $item->mutasi_out > 0 ? '-'.number_format($item->mutasi_out, 0, ',', '.') : '-' }}</div>
                                <div class="mt-1 opacity-75 text-danger fw-medium" style="font-size: 0.75rem;">Rp {{ number_format($item->nilai_out ?? 0, 0, ',', '.') }}</div>
                            </td>

                            {{-- Saldo Akhir (Qty & Rp) --}}
                            <td class="text-end pe-4 bg-primary-subtle">
                                <div class="fs-5 fw-bold d-block w-100 {{ $item->saldo_akhir < 0 ? 'text-danger' : 'text-primary' }}">
                                    {{ number_format($item->saldo_akhir, 0, ',', '.') }}
                                </div>
                                <div class="fw-bold mt-1 {{ ($item->nilai_akhir ?? 0) < 0 ? 'text-danger' : 'text-primary' }}" style="font-size: 0.85rem;">
                                    Rp {{ number_format($item->nilai_akhir ?? 0, 0, ',', '.') }}
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="7" class="py-5 text-center text-muted">
                                <i class="mb-3 opacity-25 bi bi-cash-stack display-4 d-block"></i>
                                <h6 class="fw-bold text-dark">Data Valuasi Tidak Ditemukan</h6>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
