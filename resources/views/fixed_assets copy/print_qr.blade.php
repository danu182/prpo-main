...
<head>
    ...
    <title>Cetak Label Aset - {{ $assets->first()->asset_number ?? 'Massal' }}</title>
...
<div class="preview-container">
    @foreach($assets as $asset)
        @php
            $qrData = $asset->asset_number;
            $namaBarang = $asset->name ?? optional($asset->item)->name;
            $kodeItem = optional($asset->item)->code ?? '-';
            $namaPT = optional($asset->company)->name ?? 'Milik Perusahaan';
            $snFisik = $asset->serial_number ?? '-';
            $noAkuntansi = $asset->accounting_asset_number ?? '-';
            $tglTerima = $asset->acquisition_date ? \Carbon\Carbon::parse($asset->acquisition_date)->format('d/m/Y') : ($asset->created_at ? $asset->created_at->format('d/m/Y') : '-');
        @endphp

        <div class="label-sticker">
            <div class="label-qr">
                {{-- Menggunakan API QR agar seragam --}}
            ...
            <div class="label-details">
                <div class="company-name">FIXED ASSET</div>
                ...
                <table style="width: 100%; font-size: 8.5px; line-height: 1.1; border: none;">
                    <tr>
                        ...
                        <td style="padding: 0; font-weight: bold;">: {{ $kodeItem }}</td>
                    </tr>
                    <tr>
                ...
                <div style="margin-top: 3px; padding-top: 2px; border-top: 1px dotted #ccc; font-size: 7px; color: #666;">
                    Ref: {{ isset($batchId) ? 'Batch ' . $batchId : 'Master Data Aset' }}
                </div>
            </div>
        </div>
    @endforeach
