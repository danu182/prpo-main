...
<div class="container-fluid pb-5 text-dark">
            ...
            <div class="row g-4">
                <div class="col-md-4 border-end">
                    ...
                    <table class="table mb-0 table-sm table-borderless small">
                        ...
                        <tr>
                            <td class="align-middle text-muted">Dikirim Dari Gudang</td>
                            <td class="align-middle fw-bold">:
                                @php
                                    $mutasiRtv = \App\Models\StockMutation::with('warehouse')
                                                    ->where('reference_number', $rtv->rtv_number)
                                                    ->where('type', 'OUT')
                                                    ->first();

                                    $namaGudangAsli = $mutasiRtv && $mutasiRtv->warehouse ? $mutasiRtv->warehouse->name : (optional(optional($rtv->goodsReceipt)->warehouse)->name ?? 'Gudang Utama');
                                @endphp
                                <span class="px-2 py-1 bg-white border shadow-sm badge text-secondary border-secondary-subtle ms-1">
                                    <i class="bi bi-shop text-danger me-1"></i> {{ $namaGudangAsli }}
                                </span>
                            </td>
                        </tr>
                    </table>
                        ...
                        <tr class="border-bottom">
                            ...
                            <td class="py-3 text-center fw-bold text-danger fs-5">
                                ...
                                <span class="text-nowrap fw-bold text-muted text-uppercase" style="font-size: 0.65rem;">
                                    {{ $item->getRawOriginal('uom') ?: (optional(optional($masterItem)->uom)->name ?? 'PCS') }}
                                </span>
                            </td>
    ...
    {{-- 🔥 BAGIAN LAMPIRAN DOKUMEN (QUERY LANGSUNG KE DATABASE) 🔥 --}}
    @php
        $lampirans = \DB::table('return_to_vendor_attachments')
                        ->where('return_to_vendor_id', $rtv->id)
                        ->get();
    @endphp
    ...
    <div class="mt-4 border-0 shadow-sm card rounded-4">
        <div class="py-3 bg-white card-header border-bottom">
            <h6 class="mb-0 fw-bold text-dark"><i class="bi bi-paperclip me-2 text-danger"></i>Lampiran & Bukti Retur</h6>
        </div>
        <div class="p-4 card-body">
            <div class="row g-3">
                @foreach($lampirans as $attachment)
                    @php
                    ...
                    <div class="col-xl-2 col-md-3 col-sm-4 col-6">
                        <a href="{{ asset('storage/' . $attachment->file_path) }}" target="_blank" class="text-decoration-none">
                            <div class="p-3 text-center transition-all bg-white border rounded shadow-sm h-100 border-light-subtle hover-shadow">
                                <i class="bi {{ $icon }} {{ $iconColor }} d-block mb-2" style="font-size: 2.5rem;"></i>
                                <div class="text-dark small fw-bold text-truncate" title="{{ $attachment->file_name }}">
