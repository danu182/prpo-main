...
<form action="{{ route('pr.update', $pr->pr_number) }}" method="POST" enctype="multipart/form-data" id="prForm">
    @csrf
    @method('PUT')
                ...
                <div class="p-4 bg-white card-body">
                    <div class="row g-4">
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="date" name="request_date" class="form-control border-secondary-subtle" value="{{ old('request_date', $pr->request_date) }}" required>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label fw-bold small text-dark">Dibutuhkan Pada <span class="text-danger">*</span></label>
                            <input type="date" name="need_date" class="form-control border-secondary-subtle" value="{{ old('need_date', $pr->need_date) }}" required>
                        </div>
                        <div class="col-12">
                            <label class="form-label fw-bold small text-dark">Tujuan Pengadaan Umum <span class="text-danger">*</span></label>
                            <input type="text" name="description" class="form-control border-secondary-subtle bg-light" value="{{ old('description', $pr->description) }}" required>
                        </div>
                    </div>
                </div>
            </div>
    ...
    {{-- CONTAINER BARANG (ITEMS) --}}
    <div id="items-container">
        @foreach($pr->items as $iIdx => $item)
        <div class="item-card item-row" data-index="{{ $iIdx }}">
            <div class="item-card-header">
                <h6 class="mb-0 fw-bold text-dark">
                    <span class="px-2 py-1 shadow-sm badge bg-primary rounded-pill me-2 item-number">{{ $iIdx + 1 }}</span> Rincian Barang
                </h6>
            ...
            <div class="item-card-body">
                <div class="row g-4">
                    <div class="col-md-5">
                        <label class="mb-2 small text-dark fw-bold">Pilih Barang dari Katalog <span class="text-danger">*</span></label>
                        <select name="items[{{ $iIdx }}][item_id]" class="form-select select2-item-ajax" required>
                            <option value="{{ $item->item_id }}" selected>[{{ optional($item->item)->code }}] {{ optional($item->item)->name }}</option>
                        </select>
                    </div>
                    <div class="col-md-3">
                    ...
                    <div class="col-12 mt-3">
                        ...
                        <textarea name="items[{{ $iIdx }}][specification]" rows="2" class="form-control border-secondary-subtle">{{ $item->specification }}</textarea>
                    </div>
                </div>

                {{-- SECTION VENDOR --}}
                <div class="mt-4 shadow-sm vendor-section">
                    <div class="pb-2 mb-3 d-flex justify-content-between align-items-center border-bottom">
                        <label class="small fw-bold text-dark text-uppercase"><i class="bi bi-tags-fill me-2 text-success"></i>Referensi Vendor & Penawaran</label>
                        <button type="button" class="bg-white shadow-sm btn btn-sm btn-outline-success fw-bold rounded-pill" onclick="addVendor({{ $iIdx }})">
                    ...
                    <div class="vendor-container" id="vendor-container-{{ $iIdx }}">
                        ...
                        <div class="vendor-row">
                            ...
                            <div class="row g-3 pe-4">
                                ...
                                {{-- 🔥 AREA FILE MANAGER (PREVIEW & DELETE) 🔥 --}}
                                <div class="col-lg-4 col-md-12">
                                    <div class="p-2 border rounded bg-light border-secondary-subtle">
                                        
                                        @php
                                            // Tembak Database langsung agar file pasti muncul meskipun relasi Eloquent error
                                            ...
                                            <div class="mb-2">
                                                ...
                                                @foreach($existingFiles as $file)
                                                    <div class="p-2 mb-1 bg-white border border-info border-opacity-50 rounded shadow-sm d-flex justify-content-between align-items-center file-existing-row">
                                                        ...
                                                        {{-- Hidden ID untuk mempertahankan file --}}
                        ...
                        @empty
                        {{-- Blok Kosong Jika Vendor Kosong --}}
                        <div class="vendor-row">
                            <button type="button" class="top-0 p-1 mt-2 btn btn-sm text-danger position-absolute end-0 me-2" onclick="removeVendor(this)"><i class="bi bi-x-circle-fill fs-5"></i></button>
                            <div class="row g-3 pe-4">
                                <div class="col-lg-4 col-md-6">
                                ...
                                <div class="col-lg-6 mt-2"><label class="mb-1 x-small text-muted fw-bold">Link Produk</label><input type="url" name="items[{{ $iIdx }}][vendors][0][link]" class="form-control form-control-sm"></div>
                                <div class="mt-2 col-lg-6"><label class="mb-1 x-small text-muted fw-bold">Catatan untuk Vendor</label><input type="text" name="items[{{ $iIdx }}][vendors][0][notes]" class="form-control form-control-sm"></div>
                            </div>
                        </div>
                        @endforelse
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#btn-add-item').on('click', function() {
            ...
            $newRow.find('textarea[name*="[specification]"]').attr('name', `items[${itemIdx}][specification]`);
            ...
            let masterVendorOptions = $firstRow.find('.select2-vendor').html();
            $vRow.find('.select2-vendor').html(masterVendorOptions).val('');
    ...
    function initSelect2ItemAjax(context) {
        context.find('.select2-item-ajax').select2({
            theme: 'bootstrap-5', placeholder: "-- Ketik 2 Huruf --", allowClear: true, width: '100%', minimumInputLength: 2,
            ajax: {
