            ...
            <div id="itemsContainer">
                @foreach($pr->items as $index => $item)
                    @php
                        ...
                        // 2. TENTUKAN UOM ASLI DARI PR (MENGHINDARI BUG JSON)
                        $prConvRate = 1;
                        $cleanPrUom = $baseUomName;
                        $safePrUom = $baseUomName;

                        // PRIORITAS UTAMA: Cek dari relasi uom_id
                        ...
                        // PRIORITAS KEDUA: Cek Teks Manual (Bypass Json)
                        else {
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-5">
                                            ...
                                            <div class="p-3 border rounded bg-light border-secondary-subtle shadow-sm">
                                                ...
                                                {{-- AREA UPLOAD (DESAIN DASHED BUTTON MULTI-FILE) --}}
                                                <div class="mb-2">
                                                    <span class="small fw-bold text-muted">File Lampiran PO:</span>
                                                </div>
                                                
                                                {{-- Tempat file yang diupload akan muncul berjejer di sini --}}
                                                <div id="fileListContainer_{{ $index }}" class="d-flex flex-column gap-1 mb-2"></div>
                                                <div id="hiddenFileInputs_{{ $index }}" style="display: none;"></div>
                                                
                                                <button type="button" class="btn btn-sm btn-outline-primary w-100 fw-bold py-2 mb-3 bg-white" style="border-style: dashed; border-width: 2px;" onclick="triggerFilePicker('{{ $index }}')">
                                                    <i class="bi bi-plus-lg me-1"></i> Tambah File
                                                </button>

                                                {{-- 🔥 AREA REFERENSI PR (KARTU MINI ELEGAN - REVISI ANTI KEPOTONG) 🔥 --}}
                                                @if($item->vendorQuotes && $item->vendorQuotes->count() > 0)
                                                <div class="pt-2 border-top">
                                                    <button class="btn btn-outline-secondary btn-sm w-100 rounded-3 fw-bold d-flex justify-content-between align-items-center bg-white shadow-sm" type="button" data-bs-toggle="collapse" data-bs-target="#vendorData{{ $index }}" aria-expanded="false">
                                                        <span class="text-primary"><i class="bi bi-search me-1"></i> Intip Penawaran PR</span>
                                                        <span class="badge bg-primary rounded-pill shadow-sm">{{ $item->vendorQuotes->count() }} Vendor</span>
                                                    </button>
                                                    
                                                    <div class="collapse mt-2" id="vendorData{{ $index }}">
                                                        <div class="d-flex flex-column gap-2">
                                                            @foreach($item->vendorQuotes as $vq)
                                                                ...
                                                                {{-- KARTU MINI VENDOR (LAYOUT ATAS BAWAH AGAR LUAS) --}}
                                                                <div class="bg-white border border-info border-opacity-50 rounded-3 p-2 shadow-sm">
                                                                    <div class="border-bottom border-info-subtle pb-2 mb-1">
                                                                        <div class="fw-bold text-dark mb-1" style="font-size: 0.8rem; word-break: break-word; line-height: 1.2;">
                                                                            <i class="bi bi-shop text-muted me-1"></i>{{ optional($vq->vendor)->name }}
                                                                        </div>
                                                                        <div class="text-success fw-bolder" style="font-size: 0.8rem;">
                                                                            <i class="bi bi-tag-fill me-1 opacity-75"></i>{{ $vqCurr }} {{ number_format($vq->quoted_price ?? $vq->price ?? 0, 0, ',', '.') }}
                                                                        </div>
                                                                    </div>
                                                                    <div style="font-size: 0.75rem; line-height: 1.4;" class="pt-1">
                                                                        @if($vq->reference_link)
                                                                            <a href="{{ $vq->reference_link }}" target="_blank" onclick="event.stopPropagation();" class="text-decoration-none fw-bold me-2 d-inline-block mb-1"><i class="bi bi-link-45deg"></i> Link Toko</a>
                                                                        @endif
                                                                        <span class="text-muted fst-italic d-block mb-1">{{ $vq->notes ?? 'Tidak ada catatan.' }}</span>
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <select name="po_items[{{ $index }}][uom]" class="form-select border-primary text-primary fw-bold uom-selector shadow-sm" data-current-conv="{{ $prConvRate }}" onchange="updateRowUom(this, {{ $index }})">
                                        <option value="{{ $safePrUom }}" data-conv="{{ $prConvRate }}">{{ $cleanPrUom }} @if($prConvRate>1) (Isi: {{(float)$prConvRate}}) @endif [PR]</option>
...
<script>
    ...
    function splitItem(btn) {
        ...
        clonedCard.find('input, select, textarea').each(function() {
            ...
            if(!$(this).hasClass('ckeditor-spec')) {
                $(this).removeAttr('id'); 
            }
        });
