...
<body>
    ...
    <table class="signature-table" style="width: 100%; margin-top: 50px; text-align: center; font-size: 9pt; page-break-inside: avoid;">
        <tr>
            ...
            <td style="width: {{ $tdWidth }}; vertical-align: bottom;">
                ...
                <div style="font-size: 7pt; color: #555;">{{ \Carbon\Carbon::parse($po->created_at)->format('d/m/Y') }}</div>
            </td>
            ...
            @foreach($approvals as $app)
                <td style="width: {{ $tdWidth }}; vertical-align: bottom;">
                    ...
                    @if(in_array($app->status, ['APPROVED', 'REJECTED']))
                        <div style="margin-bottom: 2px;"><u><strong>{{ optional($app->approver)->name ?? 'Nama Tidak Terdata' }}</strong></u></div>
                        <div style="font-size: 8pt; color: #555;">{{ optional($app->role)->name ?? 'Atasan' }}</div>
    ...
    <div style="margin-top: 20px; text-align: center; font-size: 8pt; color: #777;">
        <em>Dokumen ini dibuat secara elektronik (Generated by ProcureApp) dan sah sebagai bukti pemesanan barang.</em>
    </div>
