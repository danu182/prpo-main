...
<head>
    <meta charset="UTF-8">
    <title>BPR Detail - {{ $bill->bill_number }}</title>
    <style>
        ...
        .curr-txt-red { text-align: left; width: 1%; padding-right: 5px !important; color: red; }
        .num-txt { text-align: right; } .num-txt-red { text-align: right; color: red; }
    </style>
</head>
<body>
    @php
    ...
    <div class="wrapper">
        ...
        @php
            $rowspanCount = $bill->items->count() + 1; 
            foreach($bill->items as $item) {
                if((float)($item->discount_amount ?? 0) > 0) $rowspanCount++;
                if((float)($item->tax_amount ?? 0) > 0) $rowspanCount++;
            }
            if(isset($bill->charges)) $rowspanCount += $bill->charges->count();
            if(isset($bill->discounts)) $rowspanCount += $bill->discounts->count();
        @endphp

        <table class="grid">
            <thead>
                <tr>
                    <th style="width: 5%;">No</th><th style="width: 14%;">Invoices No.</th><th style="width: 29%;">Description</th>
                    <th style="width: 10%;">Reference</th><th style="width: 13%;">Unit Price</th><th style="width: 16%;">Total Amount</th><th style="width: 13%;">Account No</th>
                </tr>
            </thead>
            <tbody>
                @foreach($bill->items as $index => $item)
                    @php $qty = (float) ($item->qty ?? 1); $price = (float) ($item->price ?? $item->amount ?? 0); $discAmt = (float) ($item->discount_amount ?? 0); $taxAmt = (float) ($item->tax_amount ?? 0); @endphp
                    <tr>
                        ...
                        @if($index === 0)
                        <td rowspan="{{ $rowspanCount }}" style="text-align: center; font-weight: bold; color: #198754; vertical-align: top; padding-top: 15px;">{{ $bill->account_number ?? '-' }}</td>
                        @endif
                    </tr>
                    @if($discAmt > 0)
                    <tr>
                        <td style="border-right: none;"></td><td style="border-right: none; border-left: none;"></td>
                        <td style="color: red; border-left: none;">Diskon Item: {{ $item->name }}</td><td style="text-align: center;">1</td><td style="text-align: center;">-</td>
                ...
                @if(isset($bill->discounts)) @foreach($bill->discounts as $disc)
                <tr>
                    <td style="border-right: none;"></td><td style="border-right: none; border-left: none;"></td>
                    <td style="color: red; border-left: none;">{{ optional($disc->discountType)->name ?? 'Potongan' }}</td><td style="text-align: center;">1</td><td style="text-align: center;">-</td>
                ...
                @if(isset($bill->charges)) @foreach($bill->charges as $charge)
                <tr>
                    <td style="border-right: none;"></td><td style="border-right: none; border-left: none;"></td>
                    <td style="border-left: none;">{{ optional($charge->chargeType)->name ?? 'Biaya Tambahan' }}</td><td style="text-align: center;">1</td><td style="text-align: center;">-</td>
