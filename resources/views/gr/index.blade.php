...
<div class="container-fluid pb-5">
    ...
    <div class="gap-3 mb-4 d-flex flex-column flex-md-row justify-content-between align-items-md-center">
        ...
        <div class="gap-2 d-flex flex-column flex-md-row align-items-md-center">
            ...
            @can('manage_gi') {{-- Sesuaikan dengan permission staf gudang Anda --}}
                <button type="button" class="btn btn-success rounded-3 fw-bold shadow-sm px-4 text-nowrap" data-bs-toggle="modal" data-bs-target="#receiveModal">
            ...
            <form action="{{ route('gr.index') }}" method="GET" class="m-0 d-flex">
                <div class="shadow-sm input-group">
                    <input type="text" name="search" class="form-control" placeholder="Cari No. GR, PO, Vendor..." value="{{ request('search') }}">
                ...
                <div class="modal-body p-0 position-relative bg-light">
                    ...
                    <div class="p-3 bg-white border-bottom sticky-top" style="z-index: 10;">
                        <div class="input-group shadow-sm">
                            ...
                            <input type="text" id="searchInputModal" class="form-control border-start-0 ps-0 fw-semibold text-primary" placeholder="Ketik Nomor PO atau Nama Vendor (Pencarian Kilat)...">
                        </div>
                    </div>

                    <div class="list-group list-group-flush bg-white" id="poListContainer" style="max-height: 450px; overflow-y: auto;">
                        @forelse($readyPOs as $po)
                            <a href="{{ route('gr.create', $po->po_number) }}" class="list-group-item list-group-item-action p-4 border-bottom hover-bg-light po-item">
                                <div class="d-flex justify-content-between align-items-center">
                                    <div>
                                        <h6 class="mb-1 fw-bold text-primary">{{ $po->po_number }}</h6>
                                        <div class="text-dark fw-semibold mb-1"><i class="bi bi-shop me-1"></i> {{ optional($po->vendor)->name }}</div>
                                        <div class="small text-muted"><i class="bi bi-building me-1"></i> Tujuan: {{ optional($po->company)->name ?? 'Head Office' }}</div>
                                    </div>
                                    <div class="text-end">
                                        <span class="badge bg-success-subtle text-success border border-success-subtle rounded-pill mb-2 px-3 py-2 shadow-sm"><i class="bi bi-box-arrow-in-down me-1"></i> Siap Diterima</span>
                        ...
                        <div id="noResultMessage" class="p-5 text-center text-muted d-none">
                            ...
                            <small>Coba periksa kembali Nomor PO atau ejaan Nama Vendor.</small>
                        </div>
                    </div>
                </div>
    ...
    @if(session('success'))
        <div class="alert alert-success border-0 shadow-sm rounded-4 d-flex flex-column flex-md-row align-items-md-center justify-content-between p-4 mb-4" role="alert">
            ...
            @if(session('print_url'))
                <div class="d-flex gap-2">
                    <a href="{{ session('print_url') }}" target="_blank" class="btn btn-dark rounded-pill px-4 fw-bold shadow-sm">
    ...
    {{-- Notifikasi Error --}}
    @if(session('error'))
        <div class="shadow-sm alert alert-danger alert-dismissible fade show rounded-3 border-0 border-start border-danger border-4">
    ...
    <div class="border-0 border-4 shadow-sm card border-top border-primary rounded-3 overflow-hidden">
                ...
                <table class="table mb-0 align-middle table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="py-3 ps-4">
                                {{-- 🔥 BUG FIX: Ganti $po->po_number menjadi $gr->gr_number 🔥 --}}
                        ...
                        <tr>
                            <td colspan="8" class="py-5 text-center text-muted">
                                <i class="mb-2 bi bi-search fs-1 d-block text-secondary opacity-50"></i>
                                @if(request('search'))
